...
def main() -> None:
    course_name = "Jinja Macros and Packages"
    course_dir = Path(r"response") / course_name
    slug_path = course_dir / "slug" / (course_name + ".json")

    # Output base
    out_dir = Path(r"dbt Scripts") / course_name
    ensure_dir(out_dir)
    ...
    out_path = out_dir / (course_name + " (dbt Studio).md")
